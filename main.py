...
# ── Known HWC zones with real feature values ────────────────────
HWC_REAL_FEATURES = {
...
def get_driver(df):
    ...
    return FEATURES[int(np.argmax(np.abs(sv)))]
...
def predict(req: PredictRequest):
    ...
    if nearest:
        ...
        # Formula fallback for unknown coords
    ...
    driver = get_driver(dfx)

    return PredictResponse(
        risk=risk, probability=round(prob, 2),
